...
describe('Table', () => {
  describe('rendering data is correct', () => {
    const vm = createVue({
      template: `
        <CommonTable :tableAttrs="testAttrs" :columns="testColumns" />
      `,
      ...
      created() {
        ...
        this.testColumns = getTestColumns();
      }
    });
  ...
  describe('attributes', () => {
    ...
    it('maxHeight', done => {
      const vm = createTable({maxHeight: '134'});
      setTimeout(_ => {
    ...
    it('show-header', done => {
      const vm = createTable({showHeader: false});
      setTimeout(_ => {
    ...
    it('tableRowClassName', done => {
      const vm = createTable({rowClassName: vm.tableRowClassName}, {
        methods: {
    ...
    it('tableRowStyle[Object]', done => {
      const vm = createTable({rowStyle: { height: '60px' }}, {});
    ...
    it('tableRowStyle[Function]', done => {
      const vm = createTable({rowStyle: vm.tableRowStyle}, {
        methods: {
    ...
    it('current-row-key', done => {
      const vm = createVue({
        ...
        created() {
          this.testAttrs.data = getTestData();
        },
    ...
    it('select-on-indeterminate', done => {
      const vm = createVue({
        template: `
          <CommonTable :tableAttrs="testAttrs" :columns="testColumns" ref="myTable" />
        `,
        ...
        mounted() {
          this.$refs.myTable.table.toggleRowSelection(this.testData[0]);
        },
